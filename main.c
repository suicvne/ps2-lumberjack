...
int main(int argc, char** argv)
{
	...
	lumber_game_t lumber_game = lumber_game_create();	

	while(1)
	{
		lumber_draw(&lumber_game);
		lumber_update(&lumber_game);
	}
...
{
	/*
	if(input_is_button_down(0, PAD_START))
	{
		test_sprite->x = 70;
		test_sprite->y = 70;
	}
	...
	if(input_is_button_down(1, PAD_START))
	{
		test_sprite->x = 360;
		test_sprite->y = 360;
	}
...
int _main(int argc, char** argv)
{
	...
	printf("libpng present\n");
#endif
	u64 White = GS_SETREG_RGBAQ(0xFF,0xFF,0xFF,0x00,0x00);
	u64 BlueTrans = GS_SETREG_RGBAQ(0x00,0x00,0x00,0x20,0x00);
...
#ifdef HAVE_LIBPNG
	gsKit_texture_png(gsGlobal, &Tex1, "test_texture.png");
	//gsKit_texture_bmp(gsGlobal, &Tex1, "test_texture.bmp");
